...
export default async function ReporteClientesPage({
  ...
  for (const pedido of pedidos) {
    const actual = porCliente.get(pedido.clienteId) ?? {
    ...
    porCliente.set(pedido.clienteId, actual);
  }

  const filas = Array.from(porCliente.values()).sort((a, b) => b.total - a.total);
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      <header className="border-b border-slate-200 bg-white px-6 py-4">
        ...
        <h1 className="text-lg font-bold text-slate-900">Ventas por Cliente</h1>
      </header>
      ...
      <div className="mx-auto max-w-3xl px-6 py-8">
        ...
        <div className="mb-4 flex justify-end">
          <ExportarBotones
            ...
            columnas={[
              { header: "Cliente", accessor: (f: (typeof filas)[number]) => f.nombre },
              {
                header: "Pedidos",
